...
class BaseLatentLoader(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        if self.preload:
            ...
            assert (
                prev_vqvae is not None and prev_dataset is not None
            ), "When not preloading the lvl1 vqvae and the dataset must be set."
    ...
    def _create_latents(self):
        ...
        for batch in tqdm.tqdm(loader, "Loading music slices..."):
            music_slice, current_track_name = (
                batch["music slice"].to(self.device),
                batch["track name"][0],
            )
    ...
    def __getitem__(self, idx):
        ...
        batch = {
            "music slice": slice.to(self.device),
