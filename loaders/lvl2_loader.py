...
class Lvl2InputDataset(Dataset):
    ...
    def __init__(self,
                 lvl1_dataset: 'MP3SliceDataset',
                 lvl1_vqvae: 'Lvl1VQVariationalAutoEncoder',
                 device: str="cpu",
                 preload: bool=True,
    ...
    def _create_lvl1_latents(self):
        """
        Runs the level 1 vqvae network to create the latents and save them. TODO: Continue
        """
        
        width, length = self._compute_latent_dims()
        data_collector = torch.zeros((0, width, length)).to(self.device)
