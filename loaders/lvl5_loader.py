...
class Lvl5InputDataset(Dataset):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.lvl4_dataset = prev_dataset
        self.lvl4_vqvae = prev_vqvae
        self.device = device
        ...
        if self.preload:
            ...
            # Save pt file if it doesn't
                ...
                self.processed_slice_data, self.metadata = self._create_lvl4_latents()
            ...
            # Save pickle file if not
                ...
                if self.metadata is None:
                    _, self.metadata = self._create_lvl4_latents()
                with open(preload_metadata_file_path, 'wb') as f:
                    pickle.dump(self.metadata, f)    
                print(f'Saved music file at {preload_metadata_file_path}')
                
        else:
            assert prev_vqvae is not None and prev_dataset is not None, 'When not preloading the lvl4 vqvae and the dataset must be set.'
            
        del self.lvl4_vqvae
                
    
    @torch.no_grad()
    def _create_lvl4_latents(self):
        """
        Runs the level 4 vqvae network to create the latents and save them.
        """
        
        width, length = self._compute_latent_dims()
        data_collector = torch.zeros((0, width, length * self.collection_parameter)).to(self.device)
        loader = DataLoader(self.lvl4_dataset, batch_size=1)
        ...
        for batch in tqdm.tqdm(loader, 'Loading music slices...'):
            ...
            music_slice = music_slice.reshape((music_slice.shape[0], self.lvl4_vqvae.input_channels, -1))
            latent = self.lvl4_vqvae.encoder(music_slice)
            latent = self.lvl4_vqvae.vq_module(latent)['v_q']
    ...
    def _compute_latent_dims(self):
        ...
        length = SLICE_LENGTH
        for dim_change in self.lvl4_vqvae.channel_dim_change_list:
            length /= dim_change
        width = self.lvl4_vqvae.latent_depth
