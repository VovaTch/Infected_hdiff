...
class MusicDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.previous_dataset = (
            previous_dataset  # Previous dataset in case the data needs to be created
        )
        self.previous_vqvae = (
            previous_vqvae  # Previous vqvae in case the data needs to be created
        )
    ...
    def setup(self, stage: str):
        if stage == "fit":
            dataset = DATASETS[self.latent_level](
                prev_dataset=self.previous_dataset,
            ...
            if train_dataset_length > 10:
                ...
                _, self.eval_dataset = random_split(
                    dataset, (train_dataset_length, len(dataset) - train_dataset_length)
                )
    ...
    def get_train_dataset_length(self):
        return (
            len(self.total_dataset_len) // self.batch_size
            if len(self.total_dataset_len) % self.batch_size == 0
            else len(self.total_dataset_len) // self.batch_size + 1
        )
