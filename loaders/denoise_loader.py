...
class DenoiseDataset(Dataset):
    ...
    def _create_dataset(self):
        ...
        loader = DataLoader(
            self.music_loader, batch_size=self.loader_batch_size, shuffle=False
        )
        self.post_vqvae_data = torch.zeros_like(
            self.music_loader.processed_slice_data
        ).to(self.vqvae.device)
        with torch.no_grad():
            for idx, batch in tqdm.tqdm(enumerate(loader), "Processing raw data..."):
                ...
                self.post_vqvae_data[
                    idx * self.loader_batch_size : (idx + 1) * self.loader_batch_size, :
                ] = vqvae_outs["output"].squeeze(1)
                if idx >= 50:
                    break
    ...
    def __getitem__(self, idx):
        ...
        if self.preload:
            ...
            batch["vqvae outs"] = self.vqvae(
                self.music_loader.processed_slice_data[idx]
            )
