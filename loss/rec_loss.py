...
class RecLoss(LossBase):
    ...
    def forward(self, x, x_target):
        ...
        if self.use_tanh:
            return self._phased_loss(F.tanh(pred_slice), F.tanh(target_slice), phase_parameter=self.phase_parameter)
        else:
            return self._phased_loss(pred_slice, target_slice, phase_parameter=self.phase_parameter)
        
        
    def _phased_loss(self, x: torch.Tensor, x_target: torch.Tensor, phase_parameter: int=10):
