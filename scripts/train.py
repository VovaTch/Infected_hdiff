...
def train_encoder(args, level: int = 1):
    ...
    if args.resume is None:
        ...
        model = MultiLvlVQVariationalAutoEncoder.load_from_checkpoint(
            args.resume, **cfg, loss_obj=loss
        )
...
def train_trans_encoder(args, level: int = 1):
    ...
    # Load model with loss
    config_path_selection = {
        1: "config/lvl1_trans_config.yaml",
...
def train_denoiser(args):
    ...
    lvl1_vqvae = lvl1_vqvae.load_from_checkpoint(
        "model_weights/lvl1_vqvae.ckpt", **lvl1_cfg
    ).requires_grad_(False)
    data_module = MusicDataModule(**lvl1_cfg, latent_level=1)

    # Load model
    config_path = (
        args.config if args.config is not None else "config/denoiser_config.yaml"
    )
    cfg = load_cfg_dict(config_path)
    model = WaveUNet_Denoiser(**cfg, lvl1_vqvae=lvl1_vqvae)
    if args.resume is not None:
        model = model.load_from_checkpoint(
            args.resume, **cfg, lvl1_vqvae=lvl1_vqvae, strict=False
        )
...
def train_diff(args, level: int = 0):
    ...
    # Load model with loss
    config_path_selection = {
        0: "config/denoiser_diff_config.yaml",
    ...
    if args.resume is None:
        ...
        model = DiffusionViTSongCond.load_from_checkpoint(
            args.resume, **cfg, loss_obj=loss
        )
