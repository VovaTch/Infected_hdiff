...
class BaseNetwork(pl.LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        # Optimizers
        assert scheduler_type in [
            "none",
            "one_cycle_lr",
            "reduce_on_platou",
        ]  # TODO fix typo, program the schedulers in
    ...
    def configure_optimizers(self):
        ...
        scheduler_settings = {
            ...
            "frequency": 1,
        }  # Set the scheduler to step every batch
...
class BaseDiffusionModel(BaseNetwork):
    ...
    def __init__(
        self, scheduler: str, num_steps: int, data_multiplier: float = 1, **kwargs
    ) -> pl.LightningModule:
        # Initialize variables
        ...
        self.diffusion_constants = DiffusionConstants(
            self.num_steps, scheduler=scheduler
        )
    ...
    def denoise(
        ...
        if conditionals is not None:
            multiplied_conditionals = [
                cond * self.data_multiplier for cond in conditionals
            ]
        else:
        ...
        for time_step in reversed(range(self.num_steps)):
            ...
            running_slice = self.sample_timestep(
                running_slice, time_input, multiplied_conditionals
            )
