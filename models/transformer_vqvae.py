...
def _right_pad_if_necessary(x: torch.Tensor, patch_collection_size: int):
    ...
    x_in = F.pad(
        x_in,
        (0, patch_collection_size - (x_size[1] * x_size[2] // patch_collection_size)),
    )
    return x_in
...
class TransEncoder(nn.Module):
    def __init__(
        ...
        trans_layer = nn.TransformerEncoderLayer(
            d_model=hidden_size, dropout=0.0, nhead=n_heads, norm_first=True
        )

        # Define transformers
        self.attn_stack = nn.ModuleList(
            [
                nn.TransformerEncoder(trans_layer, num_layers=n_trans)
                for _ in range(len(dim_changes_list) + 1)
            ]
        )

        # Define fully connected layers
        self.fc_stack = nn.ModuleList(
            [nn.Linear(patch_collection_size * input_channels, hidden_size)]
            + [
                nn.Linear(hidden_size * dim_change, hidden_size)
                for dim_change in dim_changes_list
            ]
            + [
                nn.Linear(
                    hidden_size // patch_collection_size * dim_changes_list[-1],
                    output_channels,
                )
            ]
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        for trans_module, fc_module, dim_change in zip(
            self.attn_stack, self.fc_stack[:-1], self.dim_changes_list
        ):
            x = fc_module(x)
            x = F.gelu(x)
...
class TransDecoder(nn.Module):
    def __init__(
        ...
        # Define fully connected layers
        self.fc_stack = nn.ModuleList(
            [nn.Linear(output_channels * patch_collection_size, hidden_size)]
            + [
                nn.Linear(hidden_size // dim_change, hidden_size)
                for dim_change in dim_changes_list
            ]
...
class TransformerVQVAE(BaseNetwork):
    ...
    def forward(self, x: torch.Tensor):
        ...
        loss_target = {"music_slice": x_padded, "z_e": z_e}
