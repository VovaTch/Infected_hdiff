...
class WaveUNet_Denoiser(BaseNetwork):
    def __init__(
        ...
        **kwargs
        ...
        enc_channel_in = [self.num_input_channels] + [
            min(self.num_decoder_layers, (i + 1)) * self.num_filters
        ...
        dec_channel_in = [enc_channel_out[-1] * 2] + [
            enc_channel_out[-i - 1] + dec_channel_out[i - 1]
        ...
        if "mel_spec_config" in kwargs:
            ...
            self.mel_spec = MelSpectrogram(
                sample_rate=self.cfg["sample_rate"], **self.mel_spec_config
            )
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i in range(self.num_decoder_layers):
            x = F.interpolate(
                x, size=x.shape[-1] * 2, mode="linear", align_corners=True
            )
    ...
    def _crop_and_concat(self, x1: torch.Tensor, x2: torch.Tensor):
        ...
        ):  # This probably should be deleted if anyone reads this besides me.
    ...
    def training_step(self, batch, batch_idx):
        ...
        assert (
            self.lvl1_vqvae is not None
        ), "Must include lvl1 vqvae model for training."
        ...
        stft_loss = F.mse_loss(
            self._mel_spec_and_process(music_slice),
            self._mel_spec_and_process(denoised_slice),
        )
