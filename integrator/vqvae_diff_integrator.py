...
class InfectedHDiffIntegrator(Integrator):
    ...
    def __iter__(self):
        ...
        ), "Must input track length via the '__call__' method."

        # Load modules
        modules = self._load_modules()
        self.progress_tracker = [0 for _ in modules]

        # Create the initial noise
        reduction_factor = np.prod(self.reduction_list)
        self.number_of_sample_vectors = (
            self.track_length * self.sample_rate // reduction_factor
        )  # TODO: Check if works, for 6 minutes should be around ~500
        init_noise = torch.randn(
            (1, self.latent_sizes[-1], self.number_of_sample_vectors)
        ).to(self.device)
        init_noise = self._right_pad_if_necessary(
            init_noise, LATENT_LENGTH
        )  # TODO: Change LATENT_LENGTH to something not hardcoded?

        init_noise = (
            init_noise.permute((0, 2, 1)).reshape((-1, 512, 8)).permute((0, 2, 1))
        )  # 2 (3) x 8 x 512

        # Reduction list reshape, count the number of required samples per level
        self.num_samples_list = [init_noise.shape[0]]
        for reduction_ind in self.reduction_list[:-1]:
            self.num_samples_list.append(self.num_samples_list[-1] * reduction_ind)
    ...
    def _load_modules(self) -> List[nn.Module]:
        ...
        for module_name, module_cfg in self.module_cfgs.items():
            ...
            if "diff" in module_name.split("_"):
                module = DiffusionViT.load_from_checkpoint(
                    self.weight_paths[module_name], **cfg
                ).to(self.device)
                self.reduction_list.append(1)
            elif "vqvae" in module_name.split("_"):
                module = MultiLvlVQVariationalAutoEncoder.load_from_checkpoint(
                    self.weight_paths[module_name], **cfg
                ).to(self.device)
                self.latent_sizes.append(cfg["latent_depth"])
                self.reduction_list.append(
                    np.prod(cfg["channel_dim_change_list"])
                    * cfg["input_dim"]
                    // cfg["latent_depth"]
                )
            else:
    ...
    @torch.no_grad()
    def _run_modules(
        self, modules: List[Tuple[str, nn.Module]], data: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        current_module_index = len(self.num_samples_list) - len(modules)
        num_samples_total = self.num_samples_list[current_module_index]
        ...
        if "diff" in current_model[0]:
            ...
            yield num_samples_total, self.progress_tracker[current_module_index]
            self.progress_tracker[current_module_index] += data.shape[0]
            recursive_output = self._run_modules(modules[1:], output)  # BS x 8 x 512

        elif "vqvae" in current_model[0]:
            output_ind, _ = current_model[1].decode(
                data
            )  # BS x 8 x 4096 or BS x 1 x 32768
            self.progress_tracker[current_module_index] += data.shape[0]
            yield num_samples_total, self.progress_tracker[current_module_index]
            cfg = load_cfg_dict(self.module_cfgs[current_model[0]])
            output = (
                output_ind.permute((0, 2, 1))
                .reshape((cfg["input_channels"] * bs, -1, cfg["latent_depth"]))
                .permute((0, 2, 1))
            )  # BS * 8 x 8 x 512
            ...
            for batch in tqdm.tqdm(
                output_dataloader, f"Running model {current_model[0]}..."
            ):
                recursive_output_ind = self._run_modules(
                    modules[1:], batch[0]
                )  # batch[0] : 1 x 8 x 512
                recursive_output_ind = (
                    recursive_output_ind.permute((0, 2, 1))
                    .reshape((cfg["input_channels"] * bs, -1, cfg["latent_depth"]))
                    .permute((0, 2, 1))
                )  # BS * 8 x 8 x 512
                recursive_output = torch.cat(
                    (recursive_output, recursive_output_ind), dim=0
                )
