...
class InfectedHDiffIntegrator(Integrator):
    ...
    def __iter__(self):
        ...
        number_of_sample_vectors = int(
            self.track_length * 44100 / (reduction_factor * self.latent_sizes[-1] * 8)
        )  # This multiplication by 8 is a hack
        init_noise = torch.randn(
            (1, self.latent_sizes[-1], number_of_sample_vectors)
        ).to(self.device)
        ...
        for module_name, module_cfg in self.module_cfgs.items():
            ...
            for data in tqdm.tqdm(dataloader, f"Running network {module_name}"):
                ...
                if isinstance(module, DiffusionViT):
                    output = module.denoise(
                        data
                    )  # TODO: Change it such that it can output running data into an outside script
                else:
                    output, _ = module.decode(data)
                ...
                yield [
                    module_name,
                    module_idx,
                    running_data_idx / len(dataloader) * 100,
                ]
    ...
    def _create_dataloader(self, data: torch.Tensor):
        ...
        padding_size = (
            data.shape[2] % self.batch_length
            if data_flattened.shape[2] > self.batch_length
            else 0
        )
        data_padded = F.pad(
            data_flattened,
            (0, self.batch_length - padding_size if padding_size != 0 else 0),
        )
        dataset = TensorDataset(
            data_padded.permute((1, 0, 2)).reshape(
                -1,
                data.shape[1],
                self.batch_length
                if data_flattened.shape[2] > self.batch_length
